...
class ObjectAndXML
{
	...
	public function __construct($iddte, $rutEmpresa = "")
	{
		...
		$this->xml->openMemory();
		touch("procesos/xml_emitidos/$rutEmpresa/$iddte.xml");
		$uri = realpath("procesos/xml_emitidos/$rutEmpresa/$iddte.xml");
	...
	private function getObject2XML(XMLWriter $xml, $data)
	{
		foreach ($data as $key => $value) {
			if (is_object($value)) {
				...
				if ($key == "Documento" || $key == "Liquidacion" || $key == "Exportaciones") {
