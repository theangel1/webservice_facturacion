...
class XmlseclibsAdapter implements AdapterInterface
{
    ...
    public function sign(DOMDocument $data, $tipo = "", $debug = false)
    {
        ...
        $node = $data->getElementsByTagName("RespuestaDTE")->item(0);
        }elseif($tipo == "RC"){
        $node = $data->getElementsByTagName("RespuestaDTE")->item(0);
        }elseif($tipo == "RM"){
        $node = $data->getElementsByTagName("Recibo")->item(0);    
        }elseif($tipo == "RME"){
        $node = $data->getElementsByTagName("EnvioRecibos")->item(0);    
        }elseif($tipo == "Cesion"){
        $node = $data->getElementsByTagName("Cesion")->item(0);    
		$nodoid = $data->getElementsByTagName("DocumentoCesion")->item(0);
        }elseif($tipo == "DTECedido"){
        $node = $data->getElementsByTagName("DTECedido")->item(0);
		$nodoid = $data->getElementsByTagName("DocumentoDTECedido")->item(0);
        }elseif($tipo == "AEC"){
        ...
        if($tipo == "ENVIO" || $tipo == "DTE" || $tipo == "EXP" || $tipo == "LIBRO" || $tipo == "AEC"){
			$objXMLSecDSig->addReference($node, $this->digestAlgorithm, $this->transforms, array('force_uri' => TRUE, 'overwrite'=>FALSE, 'id_name'=>'ID'));    
		}else if($tipo == "DTECedido" || $tipo == "Cesion"){
			//echo "Colocando URI $tipo\n";
			$objXMLSecDSig->addReference($nodoid, $this->digestAlgorithm, $this->transforms, array('force_uri' => TRUE, 'overwrite'=>FALSE, 'id_name'=>'ID'));    
		}else{
        $objXMLSecDSig->addReference($data, $this->digestAlgorithm, $this->transforms, array('force_uri' => TRUE, 'overwrite'=>FALSE));     
        }
        
        if($tipo == "RM" || $tipo == "Cesion" || $tipo == "DTECedido"){
			//echo "Firmando $tipo\n";
			$objXMLSecDSig->sign($objKey, $node, $debug);
        }else{
