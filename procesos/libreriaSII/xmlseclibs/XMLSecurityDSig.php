...
class XMLSecurityDSig
{
    ...
    public function sign($objKey, $appendToNode = NULL,$debug = false) {
        // If we have a parent node append it now so C14N properly works
//		if($debug){
//			echo "----------------a Firmar\n";
//			var_dump($appendToNode);
//			echo "----------------fin Firma\n";
//		}
        if ($appendToNode != NULL) {
			//echo "Nodo distinto de Null\n";
            $this->resetXPathObj();
        ...
        if ($xpath = $this->getXPathObj()) {
			...
			if ($sInfo = $nodeset->item(0)) {
				...
				$sigValueNode = $this->createNewSignNode('SignatureValue', chunk_split($sigValue,76,"\n"));
	...
	$ModulusNode = $baseDoc->createElementNS(XMLSecurityDSig::XMLDSIGNS, 'Modulus', self::base64Formatting ($modulus));
	$ExponentNode = $baseDoc->createElementNS(XMLSecurityDSig::XMLDSIGNS, 'Exponent', self::base64Formatting ($exponent));
	...
	$RSAKeyValueNode->appendChild($ExponentNode);        
        ...
        foreach ($certs as $X509Cert){
            ...
            $x509CertNode = $baseDoc->createElementNS(XMLSecurityDSig::XMLDSIGNS, 'X509Certificate', chunk_split($X509Cert, 76, "\n"));
	...
	static function base64Formatting ($b64) {
		/*for ($i = 0; $i < strlen($b64); $i += 76) {
			$final .= substr($b64, $i, 76). "\n";
		}*/
		$final=chunk_split($b64, 76, "\n");
		//echo $final;
		return $final;
